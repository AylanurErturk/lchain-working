...
public class LightChainNode extends SkipNode implements LightChainInterface {
  ...
  public View updateView() {
    try {
      ...
      logger.info("view successfully updated");
      return view;
    } catch (Exception e) {
      e.printStackTrace();
      return null;
  ...
  public Block mineAttempt() {
    try {
      ...
      logger.info("Attempting to mine");
      ...
      logger.info("getting transaction batch");
  ...
  public Transaction makeTransaction(String cont) {
    try {
      ...
      while (!success && waitCount > 0) {
        ...
        logger.info("Prev found: " + lstBlk.getNumID());
        Transaction t =
      ...
      logger.info("Transaction Making Failed");
      return null;
    } catch (Exception e) {
      e.printStackTrace();
      return null;
  ...
  public Block insertGenesis() {
    ...
    insertNode(b);
    insertFlagNode(b);
    logger.info("Inserting Genesis Block " + b.getNumID());
    return b;
  }
  ...
  public Block getLatestBlock() {
    try {
      ...
      logger.info("searching for block");
  ...
  public boolean validateTransaction(Transaction t) {
    ...
    try {
      ...
      if (validated) logger.info("valid transaction");
      else logger.info("invalid transaction");
    ...
    try {
      ...
      for (int i = 0; i < ts.size(); ++i) {
        if (!isAuthenticated(ts.get(i)) /* || !isSound(ts.get(i)) */) {
          logger.info("Transaction inside block is not authentic");
  ...
  public boolean isAuthenticated(Block blk) {
    try {
      ...
      if (!hash.equals(blk.getHash())) {
        logger.info("Hash of block not generated properly");
        return false;
      }
      ...
      if (verified == false) {
        logger.info("Block does not contain signature of owner");
      }
  ...
  public boolean isConsistent(Block blk) {
    try {
      ...
      if (res == false) {
        logger.info("Block not consistent");
      }
  ...
  public boolean isSound(Transaction t) {
    try {
      ...
      if (tIdx <= bIdx) logger.info("Transaction not sound, Prev: " + tIdx + ", Latest: " + bIdx);
      else logger.info("Transaction is sound");
      return tIdx > bIdx;
    } catch (Exception e) {
      e.printStackTrace();
      return false;
  ...
  public boolean isAuthenticated(Transaction t) {
    try {
      ...
      if (!hash.equals(t.getHash())) {
        logger.info("Transaction hash value not generated properly");
        return false;
      }
      ...
      if (verified == false) {
        logger.info("Transaction does not contain signature of owner");
      }
  ...
  public PublicKey getOwnerPublicKey(int num) {
    try {
      ...
      if (owner.getNumID() != num) {
        logger.info("no node was found with given numID");
        logger.info("Expected: " + num + ", Found: " + owner.getNumID());
        return null;
      }
      ...
      if (hashedKey != num) {
        logger.info("given numID does not match with hash value of public key.");
        return null;
      }
      return pk;
    } catch (NumberFormatException e) {
